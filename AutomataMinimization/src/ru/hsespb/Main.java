...
public class Main {
    ...
    private void printAutomataText(Automata automata, String name) throws FileNotFoundException {
        ...
        writer.print(' ');
        writer.println(automata.getFinishNodes().size());

        for (int i = 0; i < automata.getSize(); i++) {
            writer.println(automata.getAdjacent(i).size());
    ...
    private void fillDifferentiatedTable(Set<Character> alphabet,
        ...
        while (!queue.isEmpty()) {
            ...
            if (!reachable[current.getKey()] || !reachable[current.getValue()]) {
                continue;
            }

            for (char c : alphabet) {
                if (!invertEdges.containsKey(current.getKey()) ||
                        !invertEdges.get(current.getKey()).containsKey(c) ||
                        !invertEdges.containsKey(current.getValue())) {
                    continue;
                }

                for (int firstPrevious : invertEdges.get(current.getKey()).get(c)) {
                    if (!reachable[firstPrevious] || !invertEdges.get(current.getValue()).containsKey(c)) {
                        continue;
                    }
                    for (int secondPrevious : invertEdges.get(current.getValue()).get(c)) {
    ...
    private boolean checkEqual(Automata first, Automata second) {
        ...
        while (!queue.isEmpty()) {
            int firstNode = queue.peek().getKey();
            int secondNode = queue.poll().getValue();
